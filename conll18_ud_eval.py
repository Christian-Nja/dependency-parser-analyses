...
ID, FORM, LEMMA, UPOS, XPOS, FEATS, HEAD, DEPREL, DEPS, MISC = range(10)	# itera sul range e assegna un elemento per variabile

# Content and functional relations	#set objects
CONTENT_DEPRELS = {
...
# Conversion methods handling `str` <-> `unicode` conversions in Python2  #Python3 renamed 'unicode' to 'str' and 'str' to 'bytes'
def _decode(text):
...
def evaluate(gold_ud, system_ud):
    ...
    def spans_score(gold_spans, system_spans, debug=False):
        correct, gi, si = 0, 0, 0
        while gi < len(gold_spans) and si < len(system_spans):
            ...
            if system_spans[si].start < gold_spans[gi].start:
                ...
                si += 1
                gi += 1
            if debug and correct == precorrect:
                print("gold:\t", end="")
                for gii in [pregi-1, pregi, pregi+1]:
                    if gii==pregi: print("[", end="")
                    print("".join(map(_encode, gold_ud.characters[ gold_spans[gii].start: gold_spans[gii].end])), end="")
                    if gii==pregi: print("]", end="")
                    print(" / ", end="")
                print("\nsystem:\t", end="")
                for sii in [presi-1, presi, presi+1]:
                    if sii==presi: print("[", end="")
                    print("".join(map(_encode, system_ud.characters[ system_spans[sii].start: system_spans[sii].end])), end="")
                    if sii==presi: print("]", end="")
                    print(" / ", end="")
                print()
                print("-----")
